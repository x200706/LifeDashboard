...
class HomeController extends AdminController
{
    ...
    protected function form()
    {
        ...
        // https://github.com/z-song/laravel-admin/issues/4169
        if ($form->isCreating()) {
            ...
            $form->select('ref_bulletin', '關聯文章')->options(Bulletin::whereNull('ref_bulletin')->pluck('title','id'))->disable();
            $form->text('title', '文章標題')->required(); //TODO 如果他是關聯文章 不可以改標題；如果是全新討論，可以自己取標題名稱
            $form->text('tag', '分類')->required();
            $form->textarea('content', '內文')->placeholder('請使用HTML')->required();
            $form->text('creator', '發文者')->default(Admin::user()->username)->readonly();
    ...
    protected function detail($id)
    {
        ...
        $show->field('tag', '分類');
        $show->field('content', '內文')->unescape();
